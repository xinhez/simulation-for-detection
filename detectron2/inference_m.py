...
test_anno = home_data_dir + '%s/test_medium.txt' % test_dataset
save_output_imgs = True

fw = open('output/log_%s_m' % test_dataset, 'w')
output_path = 'output_%s_m' % (test_dataset)

experiments = [
    # 'cut-and-paste20000_m', # 25000
    # 'mvd20_m', # 01000
    'mvd120_m', # 01000
    # 'mvd600_m', # 01900
    # 'coco20_m', # 01500
...
def inference(model_path):
    ...
    line = '%4.1f' % (inference_on_dataset(predictor.model, val_loader, evaluator)['bbox']['APm'])
    print(line)
    fw.write(line + '\n')

    if save_output_imgs:
        os.makedirs('%soutput_imgs' % (model_path.split('/')[1]), exist_ok=True)
        for d in get_dicts(test_imgs, test_anno):
            ...
            cv2.imwrite('%soutput_imgs/' % (model_path.split('/')[1])+d["file_name"].split('/')[-1], v.get_image()[:, :, ::-1])
    return evaluator.pr
...
plt.savefig('output/pr_%s_m.png' % test_dataset)
plt.show()
